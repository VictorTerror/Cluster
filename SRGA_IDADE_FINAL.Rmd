...
Este relatório tem por objetivo agrupar os estabelecimentos de internação registrados na Secretaria Municipal de Sáude de São Paulo em relação à idade dos pacientes no que diz respeito à Covid-19 com diagnóstico de SRAG, no período entre março de 2020 e setembro de 2022. Ou seja, segmentar os indivíduos (**estabelecimentos de internação**) com base nas características da faixa etária dos pacientes internados em suas dependências (**total de internados, número de óbitos, número de curados, taxa de óbitos**).
...
# dados20 <- import("dados/idade_srag_obito20.csv", header = TRUE, na.rm = TRUE) |> 
#   filter(total >= 100)
# 
# dados20[is.na(dados20)] <- 0
# 
# dados20 <- dados20 |> 
#   mutate(taxa_obito = total / 24597*100)

```

```{r}
dados_evo20 <- import("dados/obito_srag20.csv", header = TRUE, na.rm = TRUE) |> 
  filter(total >= 100)

dados_evo20 <- dados_evo20[1]



dados_idade20 <- import("dados/idade_srag_obito20.csv", header = TRUE, na.rm = TRUE)




dados20 <- left_join(dados_evo20, dados_idade20, by="estab_inter")
...
```{r}
dados20 <- dados20 |> 
            add_row(estab_inter="total_col", zero_nove=sum(dados20$zero_nove), dez_nove=sum(dados20$dez_nove), vinte_nove=sum(dados20$vinte_nove), trinta_nove=sum(dados20$trinta_nove), quarenta_nove=sum(dados20$quarenta_nove), cinquenta_nove=sum(dados20$cinquenta_nove), sessenta_nove=sum(dados20$sessenta_nove), setenta_nove=sum(dados20$cem_mais), oitenta_nove=sum(dados20$oitenta_nove), noventa_nove=sum(dados20$noventa_nove), cem_mais=sum(dados20$zero_nove), ignorada=sum(dados20$ignorada), total=sum(dados20$total))
...
idade20 <- dados20|>
    select(cinquenta_nove, sessenta_nove, oitenta_nove, noventa_nove, total, taxa_obito)
...
Nos estabelecimentos de internação selecionados, se verifica que a média da taxa de óbitos por SRAG foi de 21,91% (`r round(sd(idade20$total*100),2)`%). 
...
boxplot.stats(idade20$total)$out
...
srag4020 <- srag4020 <- dados20|>
    select(estab_inter, total)|>
    filter (total >= 0.4)
...
Observa-se que `r count(srag4020)`  estabelecimentos (`r round(count(srag4020)/count(dados20)*100, 2)`%) registraram uma taxa de óbito acima de 40% em 2020.  
...
Observa-se que `r count(srag1020)` estabelecimentos (`r round(count(srag1020)/count(dados20)*100, 2)`%) registraram uma taxa de óbito abaixo de 10% em 2020.


 

### *Boxplot* do total de obitos
...
taxa_total20 <- dados20|>
    select(estab_inter, total)|>
    filter (total >= min(boxplot.stats(idade20$total)$out))
  
kable(taxa_total20)

```

Observa-se a presença de `r count(taxa_total20)` estabelecimentos (`r round(count(taxa_total20)/count(dados20)*100, 2)`%) que registraram um número total de internações acima do padrão geral (*outliers*) em 2020.
...
idade20.padronizado <- scale(idade20[,1:ncol(idade20)])
head(idade20.padronizado)
...
wss20 <- fviz_nbclust(idade20.padronizado, kmeans, method = "wss")
...
idade20.k3 <- kmeans(idade20.padronizado, centers = 3, nstart = 25)
idade20.k4 <- kmeans(idade20.padronizado, centers = 4, nstart = 25)
idade20.k5 <- kmeans(idade20.padronizado, centers = 5, nstart = 25)
idade20.k6 <- kmeans(idade20.padronizado, centers = 6, nstart = 25)
...
G20K3 <- fviz_cluster(idade20.k3, geom = "point", data = idade20.padronizado) + ggtitle("2020 - k = 3")
G20K4 <- fviz_cluster(idade20.k4, geom = "point", data = idade20.padronizado) + ggtitle("2020 - k = 4")
G20K5 <- fviz_cluster(idade20.k5, geom = "point", data = idade20.padronizado) + ggtitle("2020 - k = 5")
G20K6 <- fviz_cluster(idade20.k6, geom = "point", data = idade20.padronizado) + ggtitle("2020 - k = 6")
...
agrupamento20 <- import("dados/idade_srag_obito20.csv", header = TRUE, na.rm = TRUE) |> 
  filter(total >= 100)

agrupamento20[is.na(agrupamento20)] <- 0

agrupamento20 <- agrupamento20 |> 
  mutate(taxa_obito = total / 24597)

agrupamento20fit <- data.frame(idade20.k4$cluster)

#Agrupar cluster e base

agrupamento20_final <-  cbind(agrupamento20, agrupamento20fit)

agrupamento20_final[order(idade20.k4$cluster, decreasing = FALSE),] |> 
  kable()
...
std.dev20  <- agrupamento20_final |> 
    group_by(idade20.k4$cluster) |>
    summarise(zero_nove = sd(zero_nove), 
              dez_nove = sd(dez_nove), 
              vinte_nove = sd(vinte_nove),
              trinta_nove = sd(trinta_nove),
              quarenta_nove = sd(quarenta_nove),
              cinquenta_nove = sd(cinquenta_nove),
              sessenta_nove = sd(sessenta_nove),
              setenta_nove = sd(setenta_nove),
              oitenta_nove = sd(oitenta_nove),
              noventa_nove = sd(noventa_nove),
              cem_mais = sd(cem_mais),
              total = sd(total),
              taxa_obito = sd(taxa_obito)
              )
...
coefvar20 <- agrupamento20_final |> 
    ...
    summarise(zero_nove = (sd(zero_nove)/mean(zero_nove)*100), 
              ...
              taxa_obito = (sd(taxa_obito)/mean(taxa_obito)*100)
              )
...
km_srag20 <- eclust(idade20.padronizado, 
                  "kmeans", 
                  k = 4,
                  nstart = 25,
                  graph = FALSE)
...
intern20 <- clValid(idade20.padronizado, nClust = 2:6, 
...
# Stability measures
clmethods20 <- c("hierarchical","kmeans","pam")
stab20 <- clValid(idade20.padronizado, nClust = 2:4, clMethods = clmethods20, 
...
cluster_srag20 <- as.numeric(idade20.k4$cluster)
...
G20 <- ggplot(dados20,
    ...
    scale_y_continuous(labels = scales::percent, breaks = seq(from = 0,to = 0.2,by = 0.01), limits = c(0,0.2))+
labs(x = "Total de pacientes internados",
     y = "Taxa de óbitos",
     caption = "Fonte = TabNet - Secretaria Municipal de Saúde de São Paulo")+
    geom_point(stat = "identity",
               aes(color = as.factor(agrupamento20_final$idade20.k4.cluster))) + scale_color_discrete(name=" ",
...
Da visualização acima, observamos que os estabelecimentos de internação se distribuem em 4 *clusters*, considerando a taxa de óbito em função do número total de internações de pacientes com covid-19 SRAG em suas dependências, em 2020,  como segue:
