...
taxa_cura20 <- dados20|>
    ...
    filter(cura >= min(boxplot.stats(evolucao20$cura)$out))

kable(taxa_cura20)

```

Observa-se a presença de `r count(taxa_cura20)` estabelecimentos (`r round(count(taxa_cura20)/count(dados20)*100, 2)`%) que registraram um número absoluto de pacientes curados acima do padrão geral (*outliers*) em 2020. 
...
taxa_out20 <- dados20|>
    ...
    filter (obito >= min(boxplot.stats(evolucao20$obito)$out))

kable(taxa_out20)

```

Observa-se a presença de `r count(taxa_out20)` estabelecimentos (`r round(count(taxa_out20)/count(dados20)*100, 2)`%) que registraram um número absoluto de óbitos acima do padrão geral (*outliers*)em 2020. 
...
taxa_total20 <- dados20|>
    select(estab_inter, total, taxa_ob)|>
    filter (total >= min(boxplot.stats(evolucao20$total)$out))
