...
labs(x = "Total de pacientes internados",
    ...
    geom_point(stat = "identity",
               aes(color = as.factor(evolucao_final$evolucao.k6.cluster))) + scale_color_discrete(name=" ",
...
Nos estabelecimentos de internação selecionados, se verifica que a média da taxa de óbitos por SRAG foi de `r round(mean(evolucao$taxa_ob)*100, 2)`% (`r round(sd(evolucao$taxa_ob*100),2)`%). 

A linha central marca a mediana do banco de dados. O valor da da mediana foi de `r round(median(evolucao$taxa_ob)*100, 2)`%.  Dito de outra maneira, em 2021, 50% dos estabelecimentos de internação da região metropolitana de São Paulo registraram uma taxa de óbito por SRAG inferior a `r round(median(evolucao$taxa_ob)*100, 2)`%.
...
Observe que 25% dos estabelecimentos de internação registraram uma taxa de óbitos por SRAG igual ou maior a `r round(quantile(evolucao$taxa_ob, probs = 0.75)*100, 2)`%. 

A taxa máxima de óbito registrada em 2021 foi de `r round(max(evolucao$taxa_ob)*100, 2)`% e não se configura como *outiler*. 
...
Observa-se que `r count(taxa40)`  estabelecimentos (`r round(count(taxa40)/count(dados21)*100, 2)`%) registraram uma taxa de óbito acima de 40% em 2021. 
...
Observa-se que `r count(taxa10)` estabelecimentos (`r round(count(taxa10)/count(dados21)*100, 2)`%) registraram uma taxa de óbito abaixo de 10% em 2021. 
...
```
Observa-se a presença de `r count(taxa_cura2)` estabelecimentos (`r round(count(taxa_cura2)/count(dados21)*100, 2)`%) que registraram um número absoluto de pacientes curados acima do padrão geral (*outliers*) em 2021. 
...
Observa-se a presença de `r count(taxa_out2)` estabelecimentos (`r round(count(taxa_out2)/count(dados21)*100, 2)`%) que registraram um número absoluto de óbitos acima do padrão geral (*outliers*)em 2021. 
...
Observa-se a presença de `r count(taxa_total2)` estabelecimentos (`r round(count(taxa_total2)/count(dados21)*100, 2)`%) que registraram um número total de internações acima do padrão geral (*outliers*) em 2021.
...
a + geom_vline(xintercept=c(0,5), linetype="dotted", color = "red" )
...
evolucao_final[order(evolucao.k5$cluster, decreasing = FALSE),] |> 
  kable()
...
clmethods <- c("hierarchical","kmeans","pam")
intern <- clValid(evolucao.padronizado, nClust = 2:5, 
              clMethods = clmethods, validation = "internal")
# Summary
summary(intern)
...
# Stability measures
clmethods <- c("hierarchical","kmeans","pam")
stab <- clValid(evolucao.padronizado, nClust = 2:5, clMethods = clmethods, 
                validation = "stability")
# Display only optimal Scores
optimalScores(stab)
...
cluster_srag <- as.numeric(evolucao.k5$cluster)
...
labs(x = "Total de pacientes internados",
     ...
     caption = "Fonte = TabNet - Secretaria Municipal de Saúde de São Paulo")+
    geom_point(stat = "identity", 
               aes(color = as.factor(evolucao_final$evolucao.k5.cluster))) + scale_color_discrete(name=" ",
                                                                               breaks=c("1", "2", "3", "4", "5","6"),
                                                                               labels=c("Cluster 1", "Cluster 2", "Cluster 3", "Cluster 4", "Cluster 5","Cluster 6")) +
...
A linha central marca a mediana do banco de dados. O valor da da mediana foi de `r round(median(evolucao$taxa_ob)*100, 2)`%.  Dito de outra maneira, em 2022, 50% dos estabelecimentos de internação da região metropolitana de São Paulo registraram uma taxa de óbito por SRAG inferior a `r round(median(evolucao$taxa_ob)*100, 2)`%.
...
A taxa máxima de óbito registrada em 2022 foi de `r round(max(evolucao$taxa_ob)*100, 2)`% e se configura como *outiler*. 
...
Observa-se que `r count(taxa40)`  estabelecimentos (`r round(count(taxa40)/count(dados22)*100, 2)`%) registraram uma taxa de óbito acima de 40% em 2022. 
...
Observa-se que `r count(taxa10)` estabelecimentos (`r round(count(taxa10)/count(dados22)*100, 2)`%) registraram uma taxa de óbito abaixo de 10% em 2022. 
...
Observa-se a presença de `r count(taxa_cura2)` estabelecimentos (`r round(count(taxa_cura2)/count(dados22)*100, 2)`%) que registraram um número absoluto de pacientes curados acima do padrão geral (*outliers*) em 2022. 
...
Observa-se a presença de `r count(taxa_out2)` estabelecimentos (`r round(count(taxa_out2)/count(dados22)*100, 2)`%) que registraram um número absoluto de óbitos acima do padrão geral (*outliers*)em 2022. 
...
Observa-se a presença de `r count(taxa_total2)` estabelecimentos (`r round(count(taxa_total2)/count(dados22)*100, 2)`%) que registraram um número total de internações acima do padrão geral (*outliers*) em 2022.
...
a + geom_vline(xintercept=c(0,4), linetype="dotted", color = "red" )
...
evolucao2 <- import("dados/obito_srag22.csv", header = TRUE, na.rm = TRUE) |> 
  filter(total >= 100)
...
evolucaofit <- data.frame(evolucao.k4$cluster)

#Agrupar cluster e base

evolucao_final <-  cbind(evolucao2, evolucaofit)

evolucao_final[order(evolucao.k4$cluster, decreasing = FALSE),] |> 
  kable()
...
km_stats <- cluster.stats(dist(dados22),  
km_srag$cluster)
...
clmethods <- c("hierarchical","kmeans","pam")
intern <- clValid(evolucao.padronizado, nClust = 2:4, 
              clMethods = clmethods, validation = "internal")
# Summary
summary(intern)
...
# Stability measures
clmethods <- c("hierarchical","kmeans","pam")
stab <- clValid(evolucao.padronizado, nClust = 2:4, clMethods = clmethods, 
                validation = "stability")
# Display only optimal Scores
optimalScores(stab)
...
cluster_srag <- as.numeric(evolucao.k4$cluster)

clust_stats <- cluster.stats(d = dist(dados22),
                             cluster_srag ,
                             km_srag$cluster)
# Corrected Rand index
...
labs(x = "Total de pacientes internados",
     ...
     caption = "Fonte = TabNet - Secretaria Municipal de Saúde de São Paulo")+
    geom_point(stat = "identity", 
               aes(color = as.factor(evolucao_final$evolucao.k4.cluster))) + scale_color_discrete(name=" ",
                                                                               breaks=c("1", "2", "3", "4"),
...
|**Estabelecimento**                            |**2020** |**2021** |**2022** |
|-----------------------------------------------|:-------:|:-------:|:-------:|
|7174691 HOSPITAL SANCTA MAGGIORE ALTO DA MOOCA |45,19%   |34,47%   |29,60%   |
|7174705 HOSPITAL SANCTA MAGGIORE ITAIM         |41,17%   |---------|10,37%   |
|9131434 HOSPITAL SANCTA MAGGIORE HIGIENOPOLIS  |43,31%   |40,48%   |6,87%    |
|7174713 HOSPITAL SANCTA MAGGIORE MOOCA         |43,26%   |39,35%   |---------|
|7174721 HOSPITAL SANCTA MAGGIORE PARAISO       |34,04%   |33,53%   |25,41%   |
|7357079 HOSPITAL SANCTA MAGGIORE PINHEIROS     |39,11%   |36,25%   |26,87%   |
|Estado de São Paulo > 100 Casos                |21,91%   |21.94%   |11,91%   |
|Estado de São Paulo Total                      |22,35%   |21,38%   |12,40%   |
...
# 
# tabela20 <- subset(tabela20, select = c(estab_inter, taxa_ob)) |>
...
# 
# 
# linha <- data.frame("Estabelecimento" = "São Paulo", "Obito_2020" = 2, "Obito_2021" = 4, "Obito_2022" = 3)
# join <- rbind(join, linha)
