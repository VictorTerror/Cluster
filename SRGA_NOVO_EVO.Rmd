...
Foram selecionados todos os estabelecimentos de internação y~i~ que registraram um total de 100 casos de SRAG , em 2020, em suas dependências. 
...
Para fins desta análise foram considerados todos os estabelecimentos de internação com um número de casos igual ou maior a 100 casos de internação/ano. Foram identificados 151 estabelecimentos de internação com esta característica.
...
A linha central marca a mediana do banco de dados. O valor da da mediana foi de 21,82%.  Dito de outra maneira, em 2020, 50% dos estabelecimentos de internação da região metropolitana de São Paulo registraram uma taxa de óbito por SRAG de 21.82%.
...
Isso é importante para evitar encontrar padrões em dados aleatórios, bem como na situação em que deseja a comparação entre dois algoritmos de agrupamento  [@kassambara2017practical]. 
...
### Coeficientes de silhueta de cada indívíduo

Abaixo segue a relação dos 10 estabelecimentos com os maiores coeficientes S-i
...
Abaixo segue a relação dos estabelecimentos com S_i negativos. 
...
Pode-se observar que dois estabelecimentos, no *cluster* 3, apresentam coeficiente de silhueta negativo. Isso significa que eles não estão no *cluster* certo.

Observa-se que estes estabelecimentos estão mais próximos do *cluster* 5 (*cluster* vizinho).
...
Nesta análise o valor é muito baixo, sugerindo que não é um bom *cluster*.
...
O índice Rand corrigido fornece uma medida para avaliar a similaridade entre duas partições, ajustadas ao acaso. Seu intervalo é de -1 (sem concordância) a 1 (concordância perfeita) [@kassambara2017practical].

```{r}
# Compute cluster stats

cluster_srag <- as.numeric(evolucao_final$ evolucao.k6.cluster)
...
Escolher o melhor método de agrupamento para um dado dado pode ser uma tarefa difícil para o analista. 
